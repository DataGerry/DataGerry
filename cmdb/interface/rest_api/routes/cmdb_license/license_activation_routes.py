...
its Base64 blob as a downloadable `text/plain` `.txt` file the admin hands to the license portal.
The license feature is on-premise only, so the route is hidden (404) whenever the process runs in
cloud or local mode
"""
...
def get_license_activation_request(request_user: CmdbUser):
    """
    HTTP `GET` route generating and downloading an offline activation request

    Generates a fresh activation request bound to this machine, persists it (superseding any prior
    pending request), and returns its Base64 blob as a downloadable `text/plain` `.txt` attachment.
    Available in the on-premise version only (404 in cloud/local mode)

    Args:
        request_user (CmdbUser): The user requesting the activation request

    Returns:
        Response: The activation-request blob as a downloadable `.txt` attachment
    """
