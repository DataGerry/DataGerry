...
`UserSettingPayload` is deliberately gone. It wrapped each stored payload entry: it took a dict
in `from_data` and handed the same dict back in `to_json`, nothing else referenced it, and its
`__slots__` declared a `name` it never assigned - so the payload entries
