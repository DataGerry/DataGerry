...
export class ProfileInfoModalComponent {
  ...
  /**
   * Whether a profile should be highlighted as a newly released feature.
   *
   * @param profileName The profile identifier
   * @returns True when the profile carries the "New" tag
   */
  public isNewProfile(profileName: string): boolean {
    return NEW_PROFILES.has(profileName);
  }
