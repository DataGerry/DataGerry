...
/**
 * These specs focus on the migration away from the deprecated
 * [formControl] + (ngModelChange) hybrid: field edits are now propagated through
 * control.valueChanges, and toggleFormControls uses { emitEvent: false } so the
 * enable/disable churn does not re-trigger those listeners.
 */
...
/** Collect every fieldChanges$ payload emitted while running `action`. */
...
describe('TextFieldEditComponent', () => {
    ...
    beforeEach(() => {
        ...
        // Call ngOnInit exactly once so value-change listeners are registered a single
        // time (a following detectChanges() would run ngOnInit again and double them).
        component.ngOnInit();
    });
    ...
    it('should propagate text control edits through valueChanges', fakeAsync(() => {
        const cases: Array<{ control: keyof TextFieldEditComponent; type: string; value: string }> = [
            { control: 'labelControl', type: 'label', value: 'New Label' },
            { control: 'descriptionControl', type: 'description', value: 'A description' },
            { control: 'regexControl', type: 'regex', value: '[a-z]+' },
    ...
    it('should propagate the name edit with the field identifier payload', fakeAsync(() => {
        const events = captureFieldChanges(component, () => component.nameControl.setValue('hostname'));

        const nameEvent = events.find(e => e.inputName === 'name');
        expect(nameEvent).toBeDefined();
        expect(nameEvent.newValue).toBe('hostname');
        expect(nameEvent.fieldName).toBe('hostname');
        flush();
    }));
    ...
    it('should keep sibling controls enabled after a non-duplicate edit', fakeAsync(() => {
        component.labelControl.setValue('Enabled');

        expect(component.descriptionControl.enabled).toBeTrue();
        expect(component.valueControl.enabled).toBeTrue();
        expect(component.helperTextControl.enabled).toBeTrue();
        flush();
    }));
    ...
    it('should flag duplicates, disable siblings and emit isDuplicate without a normal change', fakeAsync(() => {
        ...
        expect(component.valueControl.disabled).toBeTrue();
        flush();
    }));
    ...
    it('should accept re-entering the previous value even while flagged duplicate', fakeAsync(() => {
        spyOn(fieldIdentifierValidationService, 'isDuplicate').and.returnValue(true);
    ...
    it('should not emit field changes during initialization', () => {
        const fresh = TestBed.createComponent(TextFieldEditComponent).componentInstance;
    ...
    it('should disable the name control in edit mode', () => {
        const editComponent = TestBed.createComponent(TextFieldEditComponent).componentInstance;
