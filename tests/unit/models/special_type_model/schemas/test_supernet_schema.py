...
"""
Unit tests for cmdb.models.special_type_model.schemas.supernet_schema

Focuses on the required 'Type' (IPv4/IPv6) select added to the SUPERNET SpecialType to mirror the
SUBNET type field: its field definition, its placement in the Network Details section ahead of the
network range, and that it offers IPv4 then IPv6. Includes a light regression check on the
network-range field.
"""
...
IPV4_OPTION: dict[str, str] = {FieldKey.NAME: IpAddressFamily.IPV4, FieldKey.LABEL: 'IPv4'}
IPV6_OPTION: dict[str, str] = {FieldKey.NAME: IpAddressFamily.IPV6, FieldKey.LABEL: 'IPv6'}
...
def test_supernet_type_field_is_a_required_select() -> None:
    """The supernet Type field is a required SELECT field"""
    type_field: dict[str, Any] = _fields_by_name(get_supernet_schema())[SupernetField.TYPE]
...
def test_supernet_type_field_offers_ipv4_and_ipv6() -> None:
...
def test_supernet_type_field_sits_in_network_details_before_range() -> None:
    """The Type field is rendered in Network Details, ahead of the network range"""
    section: dict[str, Any] = _section_by_name(get_supernet_schema(), IpamSection.NETWORK_DETAILS)
    section_fields: list[str] = section[SectionKey.FIELDS]

    assert SupernetField.TYPE in section_fields
    assert section[SectionKey.TYPE] == SectionType.SECTION
    assert section_fields.index(SupernetField.TYPE) < section_fields.index(SupernetField.NETWORK_RANGE)
...
def test_network_range_remains_required() -> None:
    ...
    range_field: dict[str, Any] = _fields_by_name(get_supernet_schema())[SupernetField.NETWORK_RANGE]
